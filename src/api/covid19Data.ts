...
import { countriesActionCreators, CountriesAction, CountryItem, RapidCountryItem } from '../reducers/countries';
...
export async function fetchTotal(dispatch: React.Dispatch<TotalAction>) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}world?from=${yesterday.toISOString()}&to=${today.toISOString()}`, {
      "method": "GET",
    });
...
export async function fetchCountries(dispatch: React.Dispatch<CountriesAction>) {
  dispatch(countriesActionCreators.loading());
  try {
    let countries: CountryItem[] = [];
    let countriesString = await AsyncStorage.getItem(COUNTRIES_STORAGE_KEY);
    ...
    if (countriesString === null) {
      ...
      const response2 = await fetch("https://covid-19-data.p.rapidapi.com/help/countries", {
        "method": "GET",
        "headers": {
          "x-rapidapi-key": "0162f178d6msh3be422d1bbc3758p1a2e2ejsn3cb228873be2",
      ...
      countries.map(country => {
        const rapidCountries: RapidCountryItem[] = countries2.filter(value => (value.alpha2code === country.ISO2));
      ...
      await AsyncStorage.setItem(COUNTRIES_STORAGE_KEY, JSON.stringify(countries));
    } else {
...
export async function fetchCountryTotal(code: string, dispatch: React.Dispatch<CountryAction>) {
  dispatch(countryActionCreators.loading());
  try {
      const response = await fetch(`${API_BASE_URL}total/dayone/country/${code}`, {
      "method": "GET",
    });
    const totals = await response.json();
